    ...
    <div className="relative min-h-screen">
      ...
      <section className="relative pt-32 pb-16 overflow-hidden">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <Link
            href="/coverages"
            className="inline-flex items-center gap-2 text-sm text-[#57534e] hover:text-[#9a3412] transition-colors mb-8"
          >
          ...
          <div className="flex items-start gap-6 mb-8">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#9a3412] to-[#c2410c] flex items-center justify-center shadow-lg shadow-[#9a3412]/30 flex-shrink-0">
              {iconMap[coverage.icon] ?? <Shield className="w-8 h-8 text-white" />}
            </div>
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[rgba(154, 52, 18,0.1)] border border-[rgba(154, 52, 18,0.3)] mb-3">
                <span className="text-xs font-bold text-[#9a3412] uppercase tracking-wider">Coverage</span>
              </div>
              <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-[#1c1917] leading-tight">
                {coverage.title}
              </h1>
            </div>
          </div>

          <p className="text-xl text-[#57534e] leading-relaxed max-w-3xl">
      ...
      <section className="relative pb-24">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="grid md:grid-cols-2 gap-6 mb-10">
            ...
            >
              <h2 className="text-xl font-black text-[#1c1917] mb-5">What's Covered</h2>
              <ul className="space-y-3">
                {coverage.features.map((feature) => (
                  <li key={feature} className="flex items-start gap-3">
                    <CheckCircle className="w-5 h-5 text-[#9a3412] flex-shrink-0 mt-0.5" />
                    <span className="text-[#57534e] text-sm leading-relaxed">{feature}</span>
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="space-y-6">
              ...
              >
                <h2 className="text-xl font-black text-[#1c1917] mb-5">Common Claims</h2>
                <ul className="space-y-3">
                  {coverage.commonClaims.map((claim) => (
                    <li key={claim} className="flex items-start gap-3">
                      <span className="text-[#9a3412] font-black mt-0.5">→</span>
                      <span className="text-[#57534e] text-sm leading-relaxed">{claim}</span>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              >
                <p className="text-xs font-bold text-[#9a3412] uppercase tracking-wider mb-2">Typical Limits</p>
                <p className="text-[#1c1917] font-semibold text-sm">{coverage.typicalLimits}</p>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <p className="text-[#57534e] mb-6 max-w-lg mx-auto text-sm">
          ...
          {otherCoverages.length > 0 && (
            <div>
              <h2 className="text-2xl font-black text-[#1c1917] mb-6">Other Coverages</h2>
              <div className="grid sm:grid-cols-3 gap-4">
                {otherCoverages.map((c) => (
                  ...
                  >
                    <p className="font-bold text-[#1c1917] text-sm group-hover:text-[#9a3412] transition-colors mb-1">
                      {c.shortTitle}
                    </p>
                    <p className="text-[#78716c] text-xs line-clamp-2">{c.description}</p>
                  </Link>
                ))}
              </div>
            </div>
