        ...
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          ...
          <div className="mb-10">
            <div className="flex items-center gap-3 mb-4">
              ...
              <span className="text-[#78716c] text-sm flex items-center gap-1">
                <Clock className="w-4 h-4" />
                {post.readTime}
              </span>
            </div>
            <h1 className="text-4xl md:text-5xl font-black text-[#1c1917] mb-6">{post.title}</h1>
            <p className="text-[#78716c] text-sm">{post.date}</p>
          </div>
            ...
            <div className="prose prose-invert prose-lg max-w-none space-y-6">
              {post.content.map((paragraph, i) => {
                ...
                if (paragraph.match(/^\*\*\d+\./)) {
                  ...
                  return (
                    <p key={i} className="text-[#57534e] leading-relaxed">
                ...
                return (
                  <p key={i} className="text-[#57534e] leading-relaxed whitespace-pre-line">
                    {paragraph}
                  </p>
                );
              })}
          ...
          <div
            ...
            style={{
              background: "linear-gradient(135deg, rgba(154, 52, 18,0.1) 0%, rgba(154, 52, 18,0.05) 100%)",
              border: "1px solid rgba(154, 52, 18,0.2)",
            }}
          >
            <h3 className="text-2xl font-black text-[#1c1917] mb-3">Ready to Review Your Coverage?</h3>
            ...
            <div>
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                {relatedPosts.map((p) => (
                  ...
                  >
                    <span className="text-xs font-bold text-[#9a3412] uppercase tracking-wider">{p.category}</span>
                    <p className="font-bold text-[#1c1917] text-sm mt-1 group-hover:text-[#9a3412] transition-colors line-clamp-2">
                      {p.title}
                    </p>
                  </Link>
                ))}
